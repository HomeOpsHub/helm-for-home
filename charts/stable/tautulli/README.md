...
A Helm chart for Tautulli — a Plex media server monitoring and statistics tool
...
Tautulli is a powerful monitoring and statistics tool for Plex Media Server. It tracks what has been watched, who watched it, when and where they watched it, and how it was watched. Originally deployed via [ansible-nas](https://github.com/davestephens/ansible-nas), this chart migrates Tautulli to a Kubernetes-native Helm deployment.

### Features

- Detailed viewing history and statistics for your Plex server
- Real-time monitoring of current Plex activity
- Customizable notification agents (email, Discord, Slack, Telegram, etc.)
- Rich graphs and charts for media library analytics
- Newsletter generation for recently added media
...
| livenessProbe | object | httpGet on `/status` port `http` | Liveness probe configuration |
| nameOverride | string | `""` | Override the chart name |
| nodeSelector | object | `{}` | Node selector |
| persistence.config.accessModes | list | `["ReadWriteOnce"]` | PVC access modes |
| persistence.config.enabled | bool | `true` | Enable persistent storage |
...
| readinessProbe | object | httpGet on `/status` port `http` | Readiness probe configuration |
